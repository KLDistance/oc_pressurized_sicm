...
INSERT INTO coordinate_calibration (
    ...
    def insert_data(self, region_idx, frame_idx, xpos, ypos, xidx, yidx, zheight, baseline_curr, circular_buffer_pointer_idx,\
        ztrace, curr_trace):
        sqlite3.register_adapter(np.ndarray, adapt_array)
        with closing(sqlite3.connect(self.db_fp, detect_types=sqlite3.PARSE_DECLTYPES)) as connection:
            cursor = connection.cursor()
...
INSERT INTO data (
    ...
    ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?
)''', (f'px_{region_idx}_{frame_idx}_{xidx}_{yidx}', region_idx, frame_idx, xidx, yidx, xpos, ypos, zheight, baseline_curr, circular_buffer_pointer_idx, ztrace, curr_trace))
            connection.commit()
            cursor.close()
...
def db_insert_data(db_manager, region_idx, frame_idx, xpos, ypos, xidx, yidx, zheight, baseline_curr, circular_buffer_pointer_idx,\
    ztrace, curr_trace):
    db_manager.insert_data(region_idx, frame_idx, xpos, ypos, xidx, yidx, zheight, baseline_curr, circular_buffer_pointer_idx, np.array(ztrace), np.array(curr_trace))
